...
| Phase 3 | Summarization & Indexing | Planned |
| Phase 4 | Explain / Q&A | Planned |
| Phase 5 | PR Review | Planned |
| Phase 6 | Auto Documentation | Planned |
| Phase 7 | Evaluation & Guardrails | Planned |
| Phase 8 | Security & Multi-tenant | Planned |
...
```
                    ????????????????????????????????????
                    ?         FastAPI (REST API)        ?
                    ?   /repos  /analysis  /health      ?
                    ????????????????????????????????????
                               ?
              ???????????????????????????????????
              ?                ?                ?
    ?????????????????  ???????????????  ???????????????
    ?  Repo Ingestion?  ?  Static     ?  ?  (Future)   ?
    ?  (clone, scan) ?  ?  Analysis   ?  ?  LLM Layer  ?
    ?????????????????  ?  Engine     ?  ???????????????
              ?        ???????????????
              ?               ?
    ?????????????????????????????????????
    ?        Storage Layer              ?
    ?  PostgreSQL (models, graphs)      ?
    ?  Vector DB (future: embeddings)   ?
    ?????????????????????????????????????
```
...
| **GitPython** | Git repo cloning | >=3.1 |
...
| Technology | Purpose |
|-----------|---------|
| **PostgreSQL 16** | Primary data store (symbols, graphs, metadata) |
| **Redis 7** | Background job queue (future) |
| **Qdrant** | Vector database for embeddings (future) |
