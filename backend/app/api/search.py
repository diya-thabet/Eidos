...
async def search(
    ...
    if entity_type is None or entity_type == "symbol":
        hits.extend(await _search_symbols(db, snapshot_id, pattern, q, limit))
    if entity_type is None or entity_type == "summary":
        hits.extend(await _search_summaries(db, snapshot_id, pattern, q, limit))
    if entity_type is None or entity_type == "doc":
        hits.extend(await _search_docs(db, snapshot_id, pattern, q, limit))
